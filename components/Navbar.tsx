    ...
    <nav className={`fixed top-0 z-50 w-full transition-all duration-300 ${scrolled ? 'bg-black/80 backdrop-blur-md border-b border-zinc-800' : 'bg-transparent'}`} dir={dir}>
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        ...
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => setCurrentView('home')}>
           <div className="h-6 w-6 bg-gradient-to-tr from-teal-500 to-cyan-400 rounded-sm"></div>
           <span className="text-lg font-bold tracking-tight text-white hidden sm:block">NeuroLift</span>
        </div>
        ...
        <div className="flex items-center gap-4 sm:gap-6 text-sm font-medium text-zinc-400">
          {navItems.map((item) => (
            <button
              ...
              className={`transition-colors hover:text-white ${currentView === item.id ? 'text-teal-400 font-semibold' : ''}`}
            >
              {item.label}
            </button>
          ))}
